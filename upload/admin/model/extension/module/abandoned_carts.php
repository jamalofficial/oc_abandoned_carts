...
class ModelExtensionModuleAbandonedCarts extends Model {
	...
	public function getTotalCarts($data = array()) {
		...
		$total_carts_query = "SELECT count(*) AS `total` FROM (SELECT `cart_id` FROM `oc_cart` WHERE `customer_id` > 0 AND `date_added` <= DATE_SUB(NOW(), INTERVAL 5 DAY) GROUP BY `customer_id`) AS `temp_cart`";

		$result = $this->db->query($total_carts_query);
	...
	public function getCart($customer_id) {
		...
		return $customer_with_carts[$customer_id];
	}

	public function deleteCart($customer_id) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "cart` WHERE customer_id = '$customer_id'");
	}
}
